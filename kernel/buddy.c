...
int buddy_isset(char *array, int index){
  index /= 2;
  char b = array[index/8];
  char m = (1 << (index % 8));
  return (b & m) == m;
...
void buddy_set(char *array, int index){
  index /= 2;
  char b = array[index/8];
  char m = (1 << (index % 8));
  array[index/8] = (b ^ m);
...
bd_malloc(uint64 nbytes)
{
  ...
  buddy_set(bd_sizes[k].alloc, blk_index(k, p));
  for(; k > fk; k--) {
    // split a block at size k and mark one half allocated at size k-1
    // and put the buddy on the free list at size k-1
    char *q = p + BLK_SIZE(k-1);   // p's buddy
    bit_set(bd_sizes[k].split, blk_index(k, p));
    buddy_set(bd_sizes[k-1].alloc, blk_index(k-1, p));
...
bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    buddy_set(bd_sizes[k].alloc, bi);  // free p at size k
    if (buddy_isset(bd_sizes[k].alloc, buddy)) {  // is buddy allocated?
...
bd_initfree_pair(int k, int bi, int is_left) {
  int buddy = (bi % 2 == 0) ? bi+1 : bi-1;
  int free = 0;
  if(buddy_isset(bd_sizes[k].alloc, buddy)){
  //if(bit_isset(bd_sizes[k].alloc, bi) !=  bit_isset(bd_sizes[k].alloc, buddy)) {
    ...
    if (buddy_isset(bd_sizes[k].alloc, target))
    {
